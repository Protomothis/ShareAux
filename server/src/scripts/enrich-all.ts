...
async function main() {
  ...
  const rows: { id: string; source_id: string }[] = await ds.query(
  ...
  for (const [i, row] of rows.entries()) {
    try {
      ...
      const updated = await ds.query(`SELECT song_title FROM tracks WHERE id = $1`, [row.id]);
    ...
    if ((i + 1) % 10 === 0) {
      console.log(`[enrich-all] progress: ${i + 1}/${rows.length} (success: ${success}, fail: ${fail})`);
    }
  }
